...
microbenchmark::microbenchmark( duniform( seq(-4,12,by=.0001), 4, 8), times=100)
```
It was easier to write the last one and it ran faster.
...
FizzBuzz <- function(n){
  number <- (seq(n))
  result <- ifelse(number %% 3 == 0 & number %% 5 == 0, result <- "Fizz Buzz",
                   ifelse(number %% 3 == 0, result <- "Fizz",
                          ifelse(number %% 5 == 0, result <- "Buzz", result <- number)))
  return(result)
}
```
