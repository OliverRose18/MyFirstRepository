...
mdy("Sept 13, 2010.")
```
It doesn't work because September is usually abbreviated as sep, not sept.
...
mdy("S 13, 2010.")
```
The month abreviations need to be the first 3 letters of the month's name.
...
mdy("1-5-1998.")
```
1 and 5 could both be either months or days, so it could be January 5 or May 1.
...
dmy("21-5-1998.")
```
21 has to be the day because there is no 21st month.
...
```{r}
next.birthday <- birthday + years(age + 1)
next.birthday
```
...
```{r}
mdy_hm("5-8-2015 3:00 PM", tz = "US/Arizona") %>%
  with_tz(tz = "NZ")
```
...
```{r}
births <- select(Births78, date, births)
```
...
```{r}
ggplot(births, aes(date, births)) +
  geom_point(stat = "identity")
```
It looks like there are 2 separate trend lines, so it's probably split into 2 groups based on day of the week. It looks like the lower group has less dots than the higher group so I would guess it's the weekends. Maybe people are doing less stuff on the weekends, so they move around less and provoke birth less.
...
```{r}
ggplot(births, aes(date, births, color = dow)) +
  geom_point(stat = "identity")
```
